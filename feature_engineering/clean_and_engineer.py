...
FEMA Disaster Analytics & Predictive Intelligence Framework

Purpose
-------
This module transforms raw FEMA datasets into cleaned,
engineered datasets suitable for machine learning,
predictive analytics, and dashboard visualisation.

Inputs
------
data/raw/declarations.parquet
data/raw/public_assistance.parquet
data/raw/disaster_summaries.parquet
data/processed/climate_aggregated.parquet

Outputs
-------
data/processed/features.csv
data/processed/cost_aggregated.parquet

Notes
-----
All input and output paths are resolved relative to the
project root through the shared data source configuration.
Both Batch Ingestion and Kafka Streaming pipelines write
to the same unified data directory.
"""
...
# regardless of which directory it is executed from.
#
# Script location:
#     disaster-intelligence-platform/feature_engineering/clean_and_engineer.py
#
# Data directories:
#     disaster-intelligence-platform/data/raw/
#     disaster-intelligence-platform/data/processed/"

import argparse
from data_ingestion.common.data_source import (
    get_raw_directory,
)
...
def load_climate() -> pd.DataFrame:
    """
    Load engineered climate exposure datasets.

    Climate features are generated upstream from NOAA datasets and stored in
    the project's processed data directory.

    Expected inputs
    ---------------
    data/processed/
        climate_aggregated.parquet
            • disasternumber
            • wind_speed
            • flood_severity
    ...
    pandas.DataFrame
        ...
        Combined climate dataset keyed by ``disasternumber`` with the
        following columns:
        ...
        If either dataset is unavailable, an empty placeholder DataFrame
        with the expected schema is substituted so downstream feature
        engineering can continue without failure.

    Notes
    -----
    This function intentionally degrades gracefully when climate datasets
    have not yet been generated.
    """

    logging.info("Loading climate exposure datasets...")
    ...
    if storm_path.exists():
        ...
        logging.info(
            "Loaded climate_aggregated.parquet "
            "(%d rows, %d columns).",
            len(storm),
            len(storm.columns),
        )
        ...
        logging.warning(
            "climate_aggregated.parquet not found. "
            "Using empty placeholder dataset."
        )
    ...
    if rain_path.exists():
        ...
        logging.info(
            "Loaded rainfall_aggregated.parquet "
            "(%d rows, %d columns).",
            len(rain),
            len(rain.columns),
        )
        ...
        logging.warning(
            "rainfall_aggregated.parquet not found. "
            "Using empty placeholder dataset."
        )
    ...
    # ----------------------------------------------------------
    # Normalise key column
    # ----------------------------------------------------------

    for df in (storm, rain):
    ...
    logging.info(
        "Combined climate dataset: %d rows, %d columns.",
        len(climate),
        len(climate.columns),
    )
    ...
    if climate.empty:
        ...
        logging.info(
            "No climate exposure records available. "
            "Feature engineering will continue using default values."
        )

    else:

        logging.info(
            "Climate exposure data available for %d disasters.",
            climate["disasternumber"].nunique(),
        )
